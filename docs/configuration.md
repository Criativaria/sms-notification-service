...
| `PRIVATE_NETWORK_CIDRS` | Yes | — | Comma-separated CIDRs allowed to call the internal `/api/v1/sms/send` and `/internal/dlq/*` routes. Matched against the socket remote address. |
...
| `SMS_PROVIDER_TPS` | No | `10` | Positive integer. Per-provider dispatch rate limit (BullMQ worker limiter). |
...
## Advanced tunables (optional, not in the validation schema)
